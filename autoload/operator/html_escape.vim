...
function! s:convert_entities(char_vs_entity) "{{{
    return map(copy(a:char_vs_entity), '"\\&" . v:val . ";"')
endfunction "}}}
...
function! s:make_reverse_table(char_vs_entity) "{{{
    let h = {}
    for [k, v] in items(a:char_vs_entity)
...
let s:escape_table = s:convert_entities(s:char_vs_entity)
let s:unescape_table = s:make_reverse_table(s:char_vs_entity)
...
function! s:substitute(text, table) "{{{
    let text = a:text
    for [from, to] in items(a:table)
        let text = substitute(text, from, to, 'g')
    endfor
    return text
endfunction "}}}
